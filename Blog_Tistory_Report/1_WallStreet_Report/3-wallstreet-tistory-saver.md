...
- [ ] 🚨 **제목 확인:** `SEO 핵심 제목 | M월 D일 월가 마감 분석` 형식인지, 날짜가 미국 마감일(뉴욕 현지 날짜) 기준인지 확인. 형식 불일치 시 직접 수정.
...
- [ ] 🚨 **수치·내용 일관성 검증:** 본문에 등장하는 모든 절대 수치(지수 포인트, 금리 %, 환율, 유가 등)를 웹검색으로 교차확인. 수치가 실제 마감 데이터와 불일치하거나, 수치와 서술 방향이 엇갈리는 경우(예: 지수가 하락했는데 본문이 상승으로 서술) 직접 수정 후 저장. 불일치 0건 확인 후 저장 진행.
...
- **페이지명:** writer로부터 전달받은 **통합 제목**(`SEO 핵심 제목 | M월 D일 월가 마감 분석`)을 그대로 Notion 페이지명으로 사용.
- **경로 예시:** `Blog/Report/나스닥 급반등에도 Fed 경계…투자자가 놓친 3가지 신호 | 6월 16일 월가 마감 분석`
